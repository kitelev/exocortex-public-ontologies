...
def format_value(value) -> str:
    ...
    needs_quotes = (
        ':' in value or
        '?' in value or  # YAML special character
        value.startswith('[') or
        value.startswith('{') or
        value.startswith('"') or
        value.startswith("'") or
...
def get_anchor_alias(fm: dict) -> Optional[str]:
...
def get_namespace_alias(fm: dict, fallback_ns: str) -> str:
...
def get_blank_node_alias(fm: dict) -> Optional[str]:
...
def get_statement_alias(fm: dict, uuid_map: Dict[str, str]) -> Optional[str]:
