---
layout: default
title: Statistics
---

# Ontology Statistics

*Auto-generated on 2025-12-27 12:15 UTC*

## Overview

| Metric | Count |
|--------|-------|
| Total Files | **37,339** |
| Statements (triples) | 31,836 |
| Anchors (resources) | 5,503 |
| Blank Nodes | 1,016 |
| Namespaces | 28 |

## By Namespace

| Namespace | Triples | Anchors | Blank Nodes | Total Files |
|-----------|---------|---------|-------------|-------------|
| `schema` | 17,606 | 2,994 | 0 | 20,601 |
| `as` | 1,363 | 124 | 360 | 1,848 |
| `time` | 1,295 | 98 | 71 | 1,465 |
| `dcat` | 1,338 | 37 | 31 | 1,407 |
| `sh` | 1,144 | 221 | 14 | 1,380 |
| `prov` | 1,126 | 96 | 68 | 1,291 |
| `vcard` | 882 | 153 | 75 | 1,111 |
| `qudt` | 732 | 1 | 254 | 988 |
| `geosparql` | 774 | 66 | 12 | 853 |
| `org` | 746 | 46 | 25 | 818 |
| `doap` | 741 | 56 | 5 | 803 |
| `dcterms` | 700 | 99 | 0 | 800 |
| `sioc` | 658 | 99 | 0 | 758 |
| `foaf` | 620 | 75 | 0 | 696 |
| `owl` | 450 | 78 | 0 | 529 |
| `ssn` | 397 | 21 | 91 | 510 |
| `sosa` | 331 | 36 | 2 | 370 |
| `skos` | 252 | 33 | 3 | 289 |
| `adms` | 151 | 18 | 5 | 175 |
| `rdf` | 127 | 22 | 0 | 150 |
| `dc` | 107 | 15 | 0 | 123 |
| `rdfs` | 87 | 15 | 0 | 103 |
| `grddl` | 55 | 13 | 0 | 69 |
| `void` | 57 | 11 | 0 | 69 |
| `vann` | 38 | 7 | 0 | 46 |
| `geo` | 33 | 7 | 0 | 41 |
| `dcam` | 26 | 4 | 0 | 31 |
| `xsd` | 0 | 13 | 0 | 13 |
| `vs` | 0 | 1 | 0 | 2 |

## Top Predicates

| Predicate | Usage Count |
|-----------|-------------|
| `rdf:type` | 5,341 |
| `rdfs:label` | 4,916 |
| `rdfs:comment` | 4,847 |
| `schema:domainIncludes` | 2,309 |
| `schema:rangeIncludes` | 2,120 |
| `rdfs:subClassOf` | 1,393 |
| `rdfs:isDefinedBy` | 1,273 |
| `schema:isPartOf` | 1,269 |
| `schema:source` | 1,001 |
| `rdfs:range` | 675 |
| `rdfs:domain` | 590 |
| `skos:definition` | 539 |
| `rdfs:subPropertyOf` | 456 |
| `rdf:first` | 397 |
| `rdf:rest` | 397 |

## SPARQL Endpoint

The ontology data is available for client-side SPARQL queries via the [SPARQL Query Interface](sparql.html).

| Metric | Value |
|--------|-------|
| Data Format | N-Triples (`.nt`) |
| Total Triples | ~31,700 |
| File Size | ~4.2 MB |
| Query Engine | [Comunica](https://comunica.dev/) (browser-based) |
| SPARQL Version | 1.1 |

### Query Features

- **SELECT** — Tabular results with bindings
- **ASK** — Boolean yes/no queries
- **CONSTRUCT** — Create RDF graphs from patterns
- **Prefix Support** — All 28 ontology prefixes pre-loaded

### Example Queries

```sparql
# Count all classes
SELECT (COUNT(?c) AS ?count)
WHERE { ?c a owl:Class . }
```

```sparql
# Check if a resource exists
ASK { foaf:Person rdfs:label ?label . }
```

See [SPARQL Query Interface](sparql.html) for more examples.
